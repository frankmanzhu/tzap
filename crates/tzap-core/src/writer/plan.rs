...
use crate::format::{
    ArchiveWriteError, BlockKind, FormatError, BLOCK_RECORD_FRAMING_LEN, CRITICAL_RECOVERY_LOCATOR_LEN, FORMAT_VERSION, MANIFEST_FOOTER_LEN, VOLUME_HEADER_LEN,
    VOLUME_TRAILER_LEN,
};
...
pub(crate) fn build_writer_plan_from_payload(
    ...
    let (shard_file_rows, planned_index_shards) = if payload.tar_members.is_empty() {
        ...
        let planned_index_shards = build_index_shard_plaintexts(&shard_file_rows, &payload.frames, &payload.payload_objects, options)?;
    ...
    for planned in planned_index_shards {
        let compressed = compress_zstd_frame_with_jobs(&planned.plaintext, options.zstd_level, options.jobs)?;
        let object_plan = plan_encrypted_object(compressed.len(), options.index_fec_data_shards, options.index_fec_parity_shards, options)?;
...
pub(crate) fn plan_payload_stream<S: RegularFileSource>(
    ...
    for (member_index, file) in files.iter().enumerate() {
        ...
        let mut reader = StreamingMemberReader::from_source(file, &portable_metadata, layout, source_payload_size)?;
        ...
        while member_offset < member_group_size {
            ...
            let frame = loop {
                ...
                let frame = if let Some(dictionary) = dictionary {
                    compress_zstd_frame_with_dictionary_and_jobs(candidate, options.zstd_level, dictionary, options.jobs)?
                } else {
    ...
    if !envelope.plaintext.is_empty() {
        flush_payload_envelope_plan(&mut envelope, &mut payload_objects, &mut payload_block_count, next_block_index, options)?;
    }
    ...
    F: FnOnce(&mut StreamingArchiveWriter<'_, O>) -> Result<(), ArchiveWriteError>,
{
    ...
    let mut summary = writer.finish(master_key, &resolved_kdf_params, key_wrap_records, root_auth, authenticator, progress)?;
...
pub(crate) fn append_payload_frame_to_plan(state: PayloadFramePlanState<'_>, input: PayloadFramePlanInput<'_>) -> Result<(), FormatError> {
    ...
    let offset = u32_len(state.envelope.plaintext.len(), "FrameEntry.offset_in_envelope")?;
    state.envelope.plaintext.extend_from_slice(input.frame);
    state.frames.push(payload_frame_metadata(PayloadFrameMetadataInput {
    ...
    *state.next_frame_index = checked_u64_add(*state.next_frame_index, 1, "PayloadFrame.frame_index")?;
    Ok(())
}
...
pub(crate) fn flush_payload_envelope_plan(
    ...
    let object_plan = plan_encrypted_object(envelope.plaintext.len(), options.fec_data_shards, options.fec_parity_shards, options)?;
    let extent = ObjectExtent::new(*next_block_index, object_plan)?;
    *next_block_index = extent.next_block_index()?;
...
pub(crate) fn required_stripe_width_for_plan(plan: &WriterPlan, master_key: &MasterKey, target_volume_size: u64) -> Result<u32, FormatError> {
    let subkeys = writer_subkeys(master_key, plan.options.aead_algo, &plan.archive_uuid, &plan.session_id)?;
    ...
    for volume_index in 0..plan.options.stripe_width {
        let block_count = striped_block_count(plan.total_block_count, plan.options.stripe_width, volume_index);
    ...
    if target_volume_size <= max_overhead {
        return Err(FormatError::WriterUnsupported("volume-size is too small for per-volume metadata"));
    }
...
pub(crate) fn planned_v45_volume_size(plan: &WriterPlan, subkeys: &Subkeys, volume_index: u32, block_count: u64) -> Result<u64, FormatError> {
    ...
    let manifest_footer_offset = checked_u64_add(plan.block_records_offset, block_record_bytes, "volume records")?;
    ...
    O: ArchiveWriteSink,
{
    ...
    let mut summary = emit_writer_plan_suffix(&subkeys, root_auth, authenticator, plan, sink, state)?;
    summary.timings.emit_payload += emit_payload;
    Ok(summary)
}
...
pub(crate) fn emit_writer_plan_suffix<O: ArchiveWriteSink>(
    ...
    let dictionary_records = if let (Some(compressed), Some((extent, _))) = (plan.compressed_dictionary.as_ref(), plan.dictionary_extent) {
    ...
    let root_auth_footer = match root_auth {
        Some(config) => {
            let signer = authenticator.ok_or(FormatError::WriterInvariant("missing root-auth authenticator"))?;
    ...
    let root_auth_footer_length = root_auth_footer.as_ref().map(|footer| u32_len(footer.len(), "RootAuthFooterV1")).transpose()?;
    ...
    for volume_index in 0..volume_count {
        ...
        state.bytes_written[volume_index] = checked_u64_add(state.bytes_written[volume_index], MANIFEST_FOOTER_LEN as u64, "ManifestFooter")?;
        ...
        let root_auth_footer_offset = if let Some(root_auth_footer) = root_auth_footer.as_ref() {
            ...
            state.bytes_written[volume_index] = checked_u64_add(state.bytes_written[volume_index], root_auth_footer.len() as u64, "RootAuthFooterV1")?;
            Some(offset)
        } else {
            None
        };
        ...
        state.bytes_written[volume_index] = checked_u64_add(state.bytes_written[volume_index], VOLUME_TRAILER_LEN as u64, "VolumeTrailer")?;
        ...
        state.bytes_written[volume_index] = checked_u64_add(state.bytes_written[volume_index], cmra.bytes.len() as u64, "CMRA")?;

...
pub(crate) fn parse_block_region(
    ...
    if observed_count as u64 != trailer.block_count {
        return Err(FormatError::InvalidArchive("VolumeTrailer block_count does not match BlockRecord region"));
    }
    ...
    for idx in 0..observed_count {
        ...
        let expected_block_index = checked_u64_add(
            ...
            checked_u64_mul(idx as u64, volume_header.stripe_width as u64, "BlockRecord index overflow")?,
        ...
        match BlockRecord::parse(raw, block_size) {
            ...
            Err(err) if block_record_error_is_recoverable_erasure(&err) => {
                if !erased_block_indices.insert(expected_block_index) {
                    return Err(FormatError::InvalidArchive("duplicate erased BlockRecord index"));
                }
            }
    ...
    Ok(ParsedBlockRegion { blocks, erased_block_indices })
}

pub(crate) fn validate_seekable_block_region_layout(start: u64, end: u64, block_size: usize, trailer: &VolumeTrailer) -> Result<(), FormatError> {
    if end < start {
        return Err(FormatError::InvalidArchive("ManifestFooter starts before BlockRecord region"));
    }
...
pub(crate) fn block_record_error_is_recoverable_erasure(error: &FormatError) -> bool {
    matches!(
        ...
        FormatError::BadCrc { structure: "BlockRecord" }
            ...
            | FormatError::NonZeroReserved { structure: "BlockRecord" }
    )
}
...
pub(crate) fn checked_u64_mul(lhs: u64, rhs: u64, reason: &'static str) -> Result<u64, FormatError> {
    lhs.checked_mul(rhs).ok_or(FormatError::InvalidArchive(reason))
}
...
pub(crate) fn expected_stream_block_index(volume_header: &VolumeHeader, observed_block_count: u64) -> Result<u64, FormatError> {
    checked_u64_add(
        volume_header.volume_index as u64,
        checked_u64_mul(observed_block_count, volume_header.stripe_width as u64, "BlockRecord index overflow")?,
        "BlockRecord index overflow",
    )
}
...
pub(crate) fn parse_terminal_material(
    ...
    if !terminal_candidate_reaches_eof(&candidate, bytes.len())? {
        return Err(FormatError::InvalidArchive("sequential terminal does not end at EOF"));
    }
    ...
    Ok((manifest_footer, terminal.volume_trailer, terminal.root_auth_footer))
}
...
pub(crate) fn parse_terminal_material_read_at(
    ...
    if input_len >= CRITICAL_RECOVERY_LOCATOR_LEN as u64 {
        collect_v45_locator_candidate_read_at(reader, input_len - CRITICAL_RECOVERY_LOCATOR_LEN as u64, 0, context, &mut candidates);
    }
    if input_len >= LOCATOR_PAIR_LEN as u64 {
        collect_v45_locator_candidate_read_at(reader, input_len - LOCATOR_PAIR_LEN as u64, 1, context, &mut candidates);
    }
...
pub(crate) fn sequential_extract_tar_stream_with_options(bytes: &[u8], master_key: &MasterKey, options: ReaderOptions) -> Result<Vec<u8>, FormatError> {
    ...
    validate_sequential_supported_volume(&volume_header, &parsed_crypto.fixed, &parsed_crypto.extensions)?;
    ...
    let mut tar_stream_total_validator = TarStreamTotalExtractionSizeValidator::new(parsed_crypto.fixed.max_path_length, total_extraction_cap);
    ...
    validate_tar_stream_total_extraction_size(&tar_stream, parsed_crypto.fixed.max_path_length, total_extraction_cap)?;
    Ok(tar_stream)
}
...
pub(crate) fn finalize_sequential_envelope(pending: &mut PendingSequentialEnvelope, context: SequentialEnvelopeDecodeContext<'_>) -> Result<(), FormatError> {
    if !pending.saw_last_data {
...
pub(crate) fn decode_concatenated_zstd_frames_with_cap(
    ...
    while cursor < plaintext.len() {
        ...
        if let Some(dictionary) = dictionary {
            let mut decoder = zstd::stream::Decoder::with_dictionary(&plaintext[cursor..end], dictionary).map_err(|_| FormatError::ZstdDecompressionFailure)?;
            read_zstd_frame_to_capped_output(&mut decoder, output, max_output_len, tar_stream_total_validator.as_deref_mut())?;
        } else {
            let mut decoder = zstd::stream::Decoder::new(&plaintext[cursor..end]).map_err(|_| FormatError::ZstdDecompressionFailure)?;
            read_zstd_frame_to_capped_output(&mut decoder, output, max_output_len, tar_stream_total_validator.as_deref_mut())?;
        }
        cursor = end;
    }
...
impl<'a> ObjectLoadContext<'a> {
    pub(crate) fn index_root(volume_header: &'a VolumeHeader, crypto_header: &'a CryptoHeaderFixed, subkeys: &'a Subkeys, extent: ObjectExtent) -> Self {
        Self {
            volume_header,
            crypto_header,
            extent,
    ...
    pub(crate) fn index_shard(volume_header: &'a VolumeHeader, crypto_header: &'a CryptoHeaderFixed, subkeys: &'a Subkeys, entry: &ShardEntry) -> Self {
        Self {
            volume_header,
            crypto_header,
            extent: ObjectExtent {
    ...
    pub(crate) fn payload(volume_header: &'a VolumeHeader, crypto_header: &'a CryptoHeaderFixed, subkeys: &'a Subkeys, envelope: &EnvelopeEntry) -> Self {
        Self {
            volume_header,
            crypto_header,
            extent: ObjectExtent {
...
pub(crate) fn dictionary_extent_from_index_root(index_root: &IndexRoot) -> Result<ObjectExtent, FormatError> {
        ...
        || index_root.header.dictionary_decompressed_size == 0
    {
        ...
        return Err(FormatError::InvalidArchive("dictionary extent missing from IndexRoot"));
    }
    Ok(ObjectExtent {
...
pub(crate) fn load_decrypted_object_from_parts(blocks: &impl BlockProvider, context: ObjectLoadContext<'_>) -> Result<Vec<u8>, FormatError> {
...
pub(crate) fn validate_object_extent(
    ...
    if extent.data_block_count == 0 || extent.encrypted_size == 0 {
        return Err(FormatError::InvalidArchive("encrypted object has zero data blocks or size"));
    }
    if extent.data_block_count > class_data_shard_max as u32 {
        return Err(FormatError::InvalidArchive("encrypted object exceeds its class data-shard maximum"));
    }
    if extent.parity_block_count > class_parity_shard_max as u32 {
        return Err(FormatError::InvalidArchive("encrypted object exceeds its class parity-shard maximum"));
    }
    ...
    if extent.encrypted_size as usize <= crypto_header.aead_algo.tag_len() {
        return Err(FormatError::InvalidArchive("encrypted object is too small for AEAD tag"));
    }
    Ok(())
}
...
pub(crate) fn required_object_parity(data_block_count: u64, crypto_header: &CryptoHeaderFixed) -> Result<u32, FormatError> {
    ...
    Err(FormatError::InvalidArchive("parity calculation did not converge"))
}
...
pub(crate) fn verify_dense_keys<T>(entries: &BTreeMap<u64, T>, expected_count: u64, structure: &'static str) -> Result<(), FormatError> {
    if entries.len() as u64 != expected_count {
        return Err(FormatError::InvalidArchive("decoded table count does not match IndexRoot"));
    }
...
pub(crate) fn validate_envelope_frame_coverage(frames: &BTreeMap<u64, FrameEntry>, envelopes: &BTreeMap<u64, EnvelopeEntry>) -> Result<(), FormatError> {
    ...
    for envelope in envelopes.values() {
        ...
        if envelope.frame_count as u64 > frames.len() as u64 {
            return Err(FormatError::InvalidArchive("EnvelopeEntry references missing FrameEntry"));
        }
        ...
        for frame_index in first..end {
            ...
            if !accounted_frames.insert(frame_index) {
                return Err(FormatError::InvalidArchive("FrameEntry is covered by multiple EnvelopeEntries"));
            }
            ...
            if end > envelope.plaintext_size as usize {
                return Err(FormatError::InvalidArchive("FrameEntry exceeds EnvelopeEntry plaintext_size"));
            }
    ...
    for frame_index in frames.keys() {
        if !accounted_frames.contains(frame_index) {
            return Err(FormatError::InvalidArchive("FrameEntry is not covered by any EnvelopeEntry"));
        }
    }
    Ok(())
}
...
pub(crate) fn validate_global_file_table_key_step(previous: Option<&([u8; 8], Vec<u8>, u64)>, current: &([u8; 8], Vec<u8>, u64)) -> Result<(), FormatError> {
    if let Some(previous) = previous {
        if previous >= current {
            return Err(FormatError::InvalidArchive("global FileEntry rows are not sorted and unique"));
        }
    }
    Ok(())
}

pub(crate) fn validate_file_extent_coverage_ranges(extents: &[(u64, u64)], tar_len: u64) -> Result<(), FormatError> {
    let mut ranges = Vec::with_capacity(extents.len());
    for (start, len) in extents {
        let end = checked_u64_add(*start, *len, "FileEntry")?;
        if end > tar_len {
            return Err(FormatError::InvalidArchive("FileEntry extent exceeds IndexRoot tar_total_size"));
        }
        ranges.push((*start, end));
    }
    validate_exact_coverage_ranges_u64(&mut ranges, tar_len, "FileEntry extents do not cover tar stream exactly")
}

pub(crate) fn add_expected_directory_hint_rows(map: &mut DirectoryHintMap, shard_row_index: u32, path: &[u8], kind: TarEntryKind) {
    map.entry(Vec::new()).or_default().insert(shard_row_index);
    for (idx, byte) in path.iter().enumerate() {
        if *byte == b'/' {
            map.entry(path[..idx].to_vec()).or_default().insert(shard_row_index);
        }
    }
...
pub(crate) fn validate_directory_hint_tables_against_expected(tables: &[DirectoryHintTable], expected: &DirectoryHintMap) -> Result<(), FormatError> {
    ...
    for table in tables {
        for entry_index in 0..table.entries.len() {
            ...
            if let Some(previous) = &previous_key {
                if previous >= &key {
                    return Err(FormatError::InvalidArchive("DirectoryHintEntry rows are not globally sorted"));
                }
            }
    ...
    if actual != sorted_directory_hint_rows(expected) {
        return Err(FormatError::InvalidArchive("directory hint map does not match decoded files"));
    }
    Ok(())
}
...
pub(crate) fn validate_exact_coverage_ranges(ranges: &mut [(usize, usize)], expected_end: usize, reason: &'static str) -> Result<(), FormatError> {
...
pub(crate) fn validate_exact_coverage_ranges_u64(ranges: &mut [(u64, u64)], expected_end: u64, reason: &'static str) -> Result<(), FormatError> {
    ranges.sort_unstable();
    let mut cursor = 0u64;
    for (start, end) in ranges.iter().copied() {
        if start != cursor || end < start {
...
pub(crate) fn validate_non_overlapping_object_ranges(ranges: &mut [(u64, u64)]) -> Result<(), FormatError> {
    ...
    for pair in ranges.windows(2) {
        if pair[0].1 > pair[1].0 {
            return Err(FormatError::InvalidArchive("encrypted object block ranges overlap"));
        }
    }
    Ok(())
}
...
pub(crate) fn total_extraction_size_cap(options: ReaderOptions, observed_archive_bytes: u64) -> u64 {
    options.max_total_extraction_size.min(observed_archive_bytes.saturating_mul(10))
}
...
pub(crate) fn read_at_vec_unchecked(reader: &dyn ArchiveReadAt, offset: u64, len: usize) -> Result<Vec<u8>, FormatError> {
    let mut out = vec![0u8; len];
    reader.read_exact_at(offset, &mut out)?;
    Ok(out)
}
...
pub(crate) fn checked_add(lhs: usize, rhs: usize, structure: &'static str) -> Result<usize, FormatError> {
    lhs.checked_add(rhs).ok_or(FormatError::InvalidArchive(structure))
}

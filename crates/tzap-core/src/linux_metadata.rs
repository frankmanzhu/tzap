use crate::{
    canonical_base64_encode, encode_percent_name, linux_posix_acl_xattr_to_schily,
    ...
    for name in if let Some(path) = &metadata_path {
        ...
        if name_bytes == b"system.posix_acl_access" || name_bytes == b"system.posix_acl_default" {
            ...
            native.primary_pax_records.insert(
                key.into(),
                linux_posix_acl_xattr_to_schily(&value).map_err(io::Error::other)?,
            );
...
fn capture_xattr(
    native: &mut NativeFileMetadata,
    name: &[u8],
    value: Vec<u8>,
    inline_xattr_bytes: &mut usize,
) -> io::Result<()> {
    let profile = if name.starts_with(b"security.")
